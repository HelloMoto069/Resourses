...
public final class UByteArray implements Collection<UByte>, KMappedMarker {
    ...
    public static final byte m99getw2LRezQ(byte[] bArr, int i) {
        return UByte.m42constructorimpl(bArr[i]);
    }
    ...
    private static final class Iterator extends UByteIterator {
        ...
        public byte m109nextUBytew2LRezQ() {
            int i = this.index;
    ...
    public static boolean m96containsAllimpl(byte[] bArr, Collection<UByte> collection) {
        boolean z;
        Intrinsics.checkNotNullParameter(collection, "elements");
        Iterable iterable = collection;
        if (!((Collection) iterable).isEmpty()) {
            for (Object next : iterable) {
                if (!(next instanceof UByte) || !ArraysKt.contains(bArr, ((UByte) next).m91unboximpl())) {
                    z = false;
                    continue;
                } else {
                    z = true;
